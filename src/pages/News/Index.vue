...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //专题页面
      bannerClick(item){
        this.$router.push({name:'NewsSubject',query:{sid:item.sid}})
      },
      //分页
      pageShow(val){
        let self = this
    ...
    mounted(){
      let self = this
      //顶部专题
      self.$http.get(`${process.env.API.API}/news/artsub`).then(res=>{
        self.loading = true
        if(res.data.errcode=='0'){
...
//          self.newsList.page = res.data.page
      ...
      //资讯列表
      self.$http.get(`${process.env.API.API}/news/list`,{params:{rows:10,p:1}}).then(res=>{
        if(res.data.errcode=='0'){
          for(let value of res.data.data){
            value.content_pic = value.content_pic.split(',')
            self.newsList.list.push(value)
